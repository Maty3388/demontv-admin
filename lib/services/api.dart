...
class AdminApi {
  ...
  static Future<Map<String, dynamic>> addChannel(String name, String cat, String logo, String url, {int number = 0, String drmKeys = ''}) =>
      _post('/admin/channels', {'name': name, 'category': cat, 'logo': logo, 'stream_url': url, 'number': number, if (drmKeys.isNotEmpty) 'drm_keys': drmKeys});
  static Future<Map<String, dynamic>> deleteChannel(String id) => _delete('/admin/channels/$id');
  static Future<Map<String, dynamic>> updateChannel(String id, String name, String category, String logo, String streamUrl, {int number = 0, String drmKeys = ''}) => _put('/admin/channels/\$id', {"name": name, "category": category, "logo": logo, "stream_url": streamUrl, "number": number, if (drmKeys.isNotEmpty) "drm_keys": drmKeys});
